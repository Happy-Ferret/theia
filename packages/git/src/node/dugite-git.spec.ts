...
describe('git', async function () {
    ...
    describe('blame', async () => {
        ...
        it('blame file', async () => {
            ...
            await addAndCommit('six apples');

            await writeContentLines(['🍏', '🍐', '🍐', '🍏', '🍏', '🍏']);
            await addAndCommit('replace two with pears');

            await writeContentLines(['🍏', '🍐', '🍋', '🍋', '🍏', '🍏']);
            await addAndCommit('replace two with lemons');

            await writeContentLines(['🍏', '🍐', '🍋', '🍌', '🍌', '🍏']);

            await expectBlame([
                [0, 'six apples'],
                [1, 'replace two with pears'],
                [2, 'replace two with lemons'],
                [3, 'uncommited'],
                [4, 'uncommited'],
                [5, 'six apples'],
            ]);
